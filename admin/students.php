...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'activate') {
        $stmt = $pdo->prepare("UPDATE users SET is_active = 1 WHERE user_id = ? AND role = 'student'");
        $stmt->execute([$userId]);
        logActivity($_SESSION['user_id'], 'activate_student', "Activated student ID: $userId");
        $message = "Student activated successfully!";
    }
    elseif ($action === 'deactivate') {
        $stmt = $pdo->prepare("UPDATE users SET is_active = 0 WHERE user_id = ? AND role = 'student'");
        $stmt->execute([$userId]);
        logActivity($_SESSION['user_id'], 'deactivate_student', "Deactivated student ID: $userId");
        $message = "Student deactivated successfully!";
    }
    elseif ($action === 'approve') {
        $stmt = $pdo->prepare("UPDATE users SET is_approved = 1 WHERE user_id = ? AND role = 'student'");
        $stmt->execute([$userId]);
        logActivity($_SESSION['user_id'], 'approve_student', "Approved student ID: $userId");
        $message = "Student approved successfully!";
    }
    elseif ($action === 'delete') {
        $stmt = $pdo->prepare("DELETE FROM users WHERE user_id = ? AND role = 'student'");
        $stmt->execute([$userId]);
        logActivity($_SESSION['user_id'], 'delete_student', "Deleted student ID: $userId");
...
$activeStudents = $pdo->query("SELECT COUNT(*) FROM users WHERE role = 'student' AND is_active = 1")->fetchColumn();
$pendingApproval = $pdo->query("SELECT COUNT(*) FROM users WHERE role = 'student' AND is_approved = 0")->fetchColumn();
...
<div class="students-container">
    <div class="page-header">
        ...
        <p>View, activate, approve, and manage student accounts</p>
    </div>
    ...
    <?php if (empty($students)): ?>
        <div class="empty-state">
            ...
            <p>Adjust filters or wait for students to register.</p>
        </div>
    <?php else: ?>
            ...
            <table class="students-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?= htmlspecialchars($student['sector'] ?: '—') ?></td>
                        <td><?= htmlspecialchars($student['trade'] ?: '—') ?></td>
                        <td><?= $student['rqf_level'] ? 'Level ' . $student['rqf_level'] : '—' ?></td>
                        <td>
                        ...
                        <td>
                            <span class="approval-badge <?= $student['is_approved'] ? 'approved' : 'pending' ?>">
                        ...
                        <td class="actions-cell">
                            ...
                            <?php endif; ?>
                            <form method="POST" style="display:inline;" onsubmit="return confirm('Delete this student permanently? This will also delete all their progress and submissions.')">
